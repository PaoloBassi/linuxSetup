...
run_silent "Installing vim prerequisite" bash -c 'sudo apt install -y libncurses5-dev libx11-dev libxt-dev libxpm-dev libgtk-3-dev python3.13 python3.13-dev git'
...
run_silent "Generate makefile" ./configure --enable-gui=gtk3 --enable-python3interp --with-python3-command=python3.13 \
    --with-x \
